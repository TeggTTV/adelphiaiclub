...
const BoardMembers = () => {
	return (
		<section
			id="board"
			className="w-full relative py-16 sm:py-20 md:py-24 bg-black overflow-hidden"
		>
			...
			<div className="w-full container mx-auto px-4 sm:px-6 relative z-10">
				...
				<motion.div 
					className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 lg:gap-6 xl:gap-8 auto-rows-fr"
					initial="hidden"
					whileInView="visible"
				...
				>
					{boardMembers.map((member, index) => (
						<motion.div
							key={member.name}
							className="bg-white/5 backdrop-blur-md rounded-xl p-4 sm:p-6 lg:p-5 xl:p-6 border border-white/10 shadow-lg hover:shadow-2xl transform hover:scale-[1.02] transition-all duration-300 relative overflow-hidden group flex flex-col min-h-[280px] sm:min-h-[320px] lg:min-h-[300px]"
							variants={{
								hidden: { 
									opacity: 0, 
									y: 60,
						...
						>
							...
							<div className="flex flex-col items-center text-center relative z-10 flex-1">
								<motion.div 
									className="w-20 h-20 sm:w-24 sm:h-24 mx-auto mb-3 sm:mb-4 overflow-hidden rounded-full shadow-lg transform group-hover:scale-105 transition-transform duration-500 relative"
									animate={{
										y: [0, -3, 0],
										rotate: [0, 2, -2, 0]
									}}
								...
								<motion.h3
									className="text-lg sm:text-xl font-bold text-white text-center mb-1 relative"
								...
								<p className="text-gray-200 text-xs sm:text-sm leading-relaxed flex-1 flex items-center justify-center">
									{member.description}
								</p>
							</div>
						</motion.div>
